	   ...
	   char *p_commandLine)
{
  ...
  if(l_temp==NULL){
	log_error_message("AL Daemon RunAs : Cannot allocate template handler for %s !\n", p_commandLine);
	return;
  }
  ...
  if(MapUidToUser(p_euid, l_user)!=0){
	log_error_message("AL Daemon RunAs : Cannot map uid to user for %s\n", p_commandLine);
	return;
  }
  if(MapGidToGroup(p_egid, l_group)!=0){
	log_error_message("AL Daemon RunAs : Cannot map gid to user for %s\n", p_commandLine);
	return;
  }
  ...
  log_debug_message("AL Daemon RunAs : Service file was updated with User=%s and Group=%s information !\n", l_user, l_group);
  /* test application state */
  if(p_isFg==TRUE) strcpy(l_flag,"foreground");
  else strcpy(l_flag,"background");
  ...
  /* change the state of the application given by pid */
  log_debug_message("AL Daemon RunAs : Application %s will runas %s in %s \n", p_commandLine, l_user, l_flag);
...
void Suspend(int p_pid)
{
  ...
  if((l_ret=kill(p_pid, SIGSTOP))==-1){
	log_error_message("AL Daemon Suspend : %s cannot be suspended ! Err : %s\n", p_pid, strerror(errno));
   }
}
...
void Resume(int p_pid)
{
  ...
  if((l_ret=kill(p_pid, SIGCONT))==-1){
	log_error_message("AL Daemon Resume : %s cannot be resumed ! Err : %s\n", p_pid, strerror(errno));
   }
}
  ...
  if (l_ret != 0) {
	...
	("AL Daemon Stop : Application cannot be stopped with stop! Err: %s\n",
	 strerror(errno));
    	...
    	log_error_message("AL Daemon Stop : Application %s cannot be stopped because is already stopped !\n", l_commandLine); 
 }
}
...
void StopAs(int p_pid, int p_euid, int p_egid){ 
  ...
  char l_cmd[DIM_MAX];
  log_debug_message("AL Daemon StopAs : %s stopped with stopas !\n", l_app_name);
  /* form the systemd command line string */
  sprintf(l_cmd, "systemctl stop %s.service", l_commandLine);
  /* test ownership and rights before stopping application */
  log_debug_message("AL Daemon StopAs : Extracting ownership info for %s\n", l_commandLine);
  ExtractOwnershipInfo(l_user, l_group, l_srv_path);
  log_debug_message("AL Daemon StopAs : The ownership information was extracted properly [ user : %s ] and [ group : %s ]\n", l_user, l_group);
  /* extract user name and group name from uid and gid */
  ...
  if((strcmp(l_user, l_str_euid)!=0) && (strcmp(l_group, l_str_egid)!=0)){
	log_error_message("AL Daemon StopAs : The current user doesn't have permissions to stopas %s!\n", l_commandLine); 
 	return;
  }
  /* call systemd */
  l_ret = system(l_cmd);
  if (l_ret != 0) {
    log_error_message("AL Daemon StopAs : Application cannot be stopped! Err:%s\n",
		strerror(errno));
   }
  }else{
    	log_error_message("AL Daemon StopAs : Application %s cannot be stopped because is already stopped !\n", l_commandLine); 
 }
}
...
void ChangeTaskState(int p_pid, bool p_isFg)
{
  ...
  /* change the state of the application given by pid */
 log_debug_message("AL Daemon ChangeTaskState : Application with pid %d changed state to %s \n", p_pid, l_flag);
}
...
void AlReplyToMethodCall(DBusMessage * p_msg, DBusConnection * p_conn)
{
  ...
  if (!dbus_message_iter_init(p_msg, &l_args)) {
    log_error_message
	("AL Daemon Reply to Method Call : Message has no arguments!\n", 0);
  } else{ 
  ...
  if ((DBUS_TYPE_STRING == dbus_message_iter_get_arg_type(&l_args))) {
   ...
   /* setup the arg type switch */
	...
	("AL Daemon Reply to Method Call : Method called for %d\n",
     ...
     log_error_message
	("AL Daemon Reply to Method Call : Argument is not string nor an int !\n", 0);
     }
  }
  ...
  if (!dbus_message_iter_append_basic(&l_args, DBUS_TYPE_BOOLEAN, &l_stat)) {
    log_error_message
	("AL Daemon Reply to Method Call : Arg Bool Out Of Memory!\n", 0);
    return;
  }
  if (!dbus_message_iter_append_basic(&l_args, DBUS_TYPE_UINT32, &l_level)) {
    log_error_message
	("AL Daemon Reply to Method Call : Arg Int Out Of Memory!\n", 0);
    return;
  }
  /* send the reply && flush the connection */
  if (!dbus_connection_send(p_conn, l_reply, &l_serial)) {
    log_error_message
	("AL Daemon Reply to Method Call : Connection Out Of Memory!\n", 0);
    return;
  }
...
void ReplyToIntrospect(DBusMessage *p_msg, DBusConnection *p_conn)
{
    ...
    if (!dbus_message_iter_append_basic(&l_args, DBUS_TYPE_STRING, &l_introspect_xml)) {
        log_debug_message("Reply to introspect : Arg String Out Of Memory!\n", 0);
        exit(1);
    }
    // send the reply && flush the connection
    if (!dbus_connection_send(p_conn, l_reply, &l_serial)) {
        log_debug_message("Reply to introspect : Connection Out Of Memory!\n", 0);
        exit(1);
    }
    dbus_connection_flush(p_conn);
